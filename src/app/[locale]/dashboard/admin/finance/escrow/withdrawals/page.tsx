...
export default function EscrowWithdrawalsPage() {
    const t = useTranslations('Admin.FinanceWithdrawals');
    ...
    const fetchData = useCallback(async () => {
        try {
            const url = filter
                ? `/api/admin/escrow/withdrawals?status=${filter}`
                : '/api/admin/escrow/withdrawals';
    ...
    async function handleApprove(id: string) {
        setProcessing(id);
        try {
            const res = await fetch(`/api/finance/withdrawals/${id}/approve`, {
                method: 'POST',
    ...
    async function handleReject(id: string) {
        ...
        setProcessing(id);
        try {
            const res = await fetch(`/api/finance/withdrawals/${id}/reject`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                        ...
                        <div className="text-sm text-emerald-600">{t('stats.totalWithdrawn')}</div>
                    </div>
                </div>
            )}
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('table.bank')}</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('table.account')}</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {requests.map(req => (
                                <tr key={req.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => { setSelectedReq(req); setInternalNote(''); }}>
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-600">{req.bankName}</td>
                                    <td className="px-4 py-3">
                                        <div className="text-sm text-gray-900">{req.accountName}</div>
                                        <div className="text-xs text-gray-400 font-mono">{req.accountNumber}</div>
                                    </td>
                                    <td className="px-4 py-3">
                                        {req.proofUrl ? (
                                            ...
                                            <span className="text-xs text-gray-400">{t('table.noProof')}</span>
                                        )}
                                    </td>
                                    <td className="px-4 py-3">
                    ...
                    <div className="w-full max-w-lg bg-white rounded-xl shadow-xl max-h-[90vh] overflow-y-auto m-4">
                        ...
                        <div className="p-6 space-y-5">
                            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                                ...
                                <div className="flex justify-between"><span className="text-xs text-gray-500">{t('modal.bank')}</span><span className="text-sm">{selectedReq.bankName}</span></div>
                                <div className="flex justify-between"><span className="text-xs text-gray-500">{t('modal.account')}</span><span className="text-sm">{selectedReq.accountName} • {selectedReq.accountNumber}</span></div>
