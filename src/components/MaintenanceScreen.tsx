        ...
        <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 p-6">
            <div className="max-w-lg text-center">
                <div className="mb-8 inline-flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-amber-500/20 to-orange-500/20 border border-amber-500/30">
                    <svg className="h-12 w-12 text-amber-400 animate-spin" style={{ animationDuration: '3s' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p className="text-gray-400 mb-2 text-lg">{t("subtitle")}</p>
                <p className="text-gray-500 mb-8 text-sm">{t("apology")}</p>
                ...
                <div className="flex flex-col gap-3">
                    <button onClick={() => window.location.reload()} className="w-full rounded-xl bg-white/10 border border-white/10 px-5 py-3.5 font-semibold text-white hover:bg-white/20 active:scale-[0.98] transition-all backdrop-blur-sm">
                        {t("checkAgain")}
                    </button>
                    <Link href="/" className="text-sm font-medium text-gray-500 hover:text-gray-300 transition-colors">
                        {t("returnHome")}
                    </Link>
                </div>

                <p className="mt-10 text-xs text-gray-600">{t("needHelp")}</p>
            </div>
        </main>
    );
}
