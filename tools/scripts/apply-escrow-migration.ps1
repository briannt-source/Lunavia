# PowerShell script to apply escrow system migration
# This script reads DATABASE_URL from .env and applies the SQL migration

$ErrorActionPreference = "Stop"

Write-Host "Applying escrow system migration..." -ForegroundColor Cyan
...
$migrationDir = Join-Path $migrationDir "20250115000005_add_escrow_system"
$migrationFile = Join-Path $migrationDir "migration_fixed.sql"

if (-not (Test-Path $migrationFile)) {
    # Use original migration file if fixed doesn't exist
